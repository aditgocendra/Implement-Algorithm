...
func _ready():
	full_cell = get_edge_pos(tile.get_used_cells())
	cell = tile.get_used_cells_by_id(0)
	edge_cell = get_pos_map(cell)
	

func get_pos_map(map_cell):
	var result = []
	for pos_map in map_cell:
		result.append(tile.map_to_world(pos_map))
	

	return result


func get_edge_pos(map_cell):
	var result = []
	for map in map_cell:
		var pos = tile.map_to_world(map)
		result.append(Vector2(pos.x + 1, pos.y + 1))
		result.append(Vector2(pos.x + 31, pos.y + 1))
		result.append(Vector2(pos.x + 1, pos.y + 31))
		result.append(Vector2(pos.x + 31, pos.y + 31))
	return result
...
func _input(event):
	if event is InputEventMouseButton:
		if event.is_pressed():
			coord = tile.world_to_map(event.position) 
			end_current_node = tile.map_to_world(coord)
		
			if edge_cell.has(end_current_node):
				start_current_node = tile.map_to_world(tile.world_to_map($Enemy.global_position))
...
#			return
		...
		var childern = PoolVector2Array([
				Vector2(current_node.tile_pos.x + 31, current_node.tile_pos.y),
				Vector2(current_node.tile_pos.x - 31,current_node.tile_pos.y),
				Vector2(current_node.tile_pos.x, current_node.tile_pos.y + 31),
				Vector2(current_node.tile_pos.x, current_node.tile_pos.y - 31),
				Vector2(current_node.tile_pos.x + 31, current_node.tile_pos.y + 31),
				Vector2(current_node.tile_pos.x - 31, current_node.tile_pos.y - 31),
				Vector2(current_node.tile_pos.x - 31, current_node.tile_pos.y + 31),
				Vector2(current_node.tile_pos.x + 31, current_node.tile_pos.y - 31)
			])
		...
		for i in range(childern.size()):
			if cell.has(tile.world_to_map(childern[i])):
...
func isBlocked(vector):
	...
	if vector.x < 0 or vector.y < 0:
		return true
	if !cell.has(tile.world_to_map(vector)):
		return true
