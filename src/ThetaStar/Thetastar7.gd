...
func line_of_sight(current, neighbour):
	...
	var current_pos_map = tile.map_to_world(current.tile_pos)
	var neighbour_pos_map = tile.map_to_world(neighbour.tile_pos)
	var x0 = current_pos_map.x + 16
	var y0 = current_pos_map.y + 16
	var x1 = neighbour_pos_map.x + 16
	var y1 = neighbour_pos_map.y + 16
	...
	if dx >= dy:
		while x0 != x1:
			f = f + dy
			if f >= dx:
				if isBlocked(Vector2(x0 + sx  , y0 + sy)): 
					return false
				y0 = y0 + sy
				f = f - dx
			
			if f != 0 and isBlocked(Vector2(x0 + sx, y0 + sy)):
				return false
			
			if dy == 0 and isBlocked(Vector2(x0 + sx, y0)) and isBlocked(Vector2(x0 + sx , y0 - 1)):
				return false
		...
		while y0 != y1:
			...
			if dx == 0 and isBlocked(Vector2(x0 , y0 + sy)) and isBlocked(Vector2(x0 - 1 , y0 + sy)):
				return false
			y0 = y0  + sy
...
func isBlocked(vector):
	...
	var map = tile.world_to_map(vector)
